...
def pipeline_api(
    ...
    parsed_doc = partition_receipts(
        file=file, filename=filename, file_content_type=file_content_type
    )
    if m_include_fields:
...
@router.post("/receipts/v0.1.0/receipts")
@limiter.limit(RATE_LIMIT)
async def pipeline_1(
    request: Request,
    files: Union[List[UploadFile], None] = File(default=None),
    cleaning_fnc: List[str] = Form(default=[]),
    include_fields: List[str] = Form(default=[]),
):
